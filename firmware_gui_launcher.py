...
tk.Checkbutton(root, text="Send STIX Alert to Dashboard", variable=dash_var).grid(row=3, column=0, columnspan=2, sticky="w", padx=5)

tk.Button(root, text="Capture PCAP", command=run_pcap_capture, bg="#4CAF50", fg="white", width=20).grid(row=4, column=0, padx=5, pady=10)
tk.Button(root, text="Analyze Firmware (STIX)", command=run_stix_export, bg="#2196F3", fg="white", width=25).grid(row=4, column=1, padx=5, pady=10)

output_text = scrolledtext.ScrolledText(root, width=95, height=25)
output_text.grid(row=5, column=0, columnspan=3, padx=5, pady=10)

tk.Label(root, text="Satellite Defense Toolkit - GUI Interface", fg="gray").grid(row=6, column=0, sticky="w", padx=5)
