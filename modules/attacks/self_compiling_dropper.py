...
cs_path = Path("StealthLoader_final_filled.cs")
cs_path.write_text(cs_code)

# Compile using CSC (assumes it's available on Windows)
output_exe = "stealth_loader.exe"
compile_cmd = f'csc /unsafe /platform:x64 /target:exe /out:{output_exe} {cs_path.name}'
subprocess.run(compile_cmd, shell=True)

print(f"â Compiled to {output_exe}")
